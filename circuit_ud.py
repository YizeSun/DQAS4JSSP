...
def circuit(cm:Circuit_manager, data_reuploading=False, barrier=False): # keep it outside DQN
        ...
        if cm.noisy:
            dev = qml.device("default.mixed", wires=cm.num_qubits, shots=1000)
        else:
            dev = qml.device("default.qubit", wires=cm.num_qubits, shots=1000)
        ...
        def layer(theta_weight, phi_weight, delta_weight, generators, generators_range, indexs):
            ...
            for p, op in enumerate(generators):
                ...
                # -- two qubits gate --
                    ...
                    for i in op_range:
                        qml.CZ(wires=[i, (i + 1) % cm.num_qubits])
                elif op == "CRZ":
                    for i in op_range:
                        qml.CRZ(theta_weight[(i + 1) % len(op_range), p, indexs[p]], wires=[i, (i + 1) % cm.num_qubits])
                elif op == "CNOT":
                    # print(op_range)
                    # assert(len(op_range) > 1)
        ...
        def make_circuit(inputs, theta_weights, phi_weights, delta_weights):
            ...
            for l in range(cm.num_layers):
                if data_reuploading or l==0:  #第一次/每一次
...
class QDQN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for i, (x, p) in enumerate(counts.items()):
            values[i] = JSSP.eval_bitstring(JSSP.transformBits(x))
            probabilities[i] = p / JSSP.num_shots

        # evaluate cvar
        cvar = JSSP.compute_cvar(probabilities, values, 0.5)
        cvar = torch.Tensor([cvar])

        return cvar.requires_grad_(True)
