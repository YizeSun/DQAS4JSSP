...
class DQAS4RL:
    ...
    def config(self):
        ...
        # set loss functions
        self.loss_func = getattr(nn, self.loss_func_name + 'Loss')().to(self.device)
    ...
    def train_model(self, prob):
        ...
        self.qdqn.train().to(self.device)
        self.opt.zero_grad()
        # TODO usefull?
        ...
        if self.prob_max and self.struc_learning:
            prob = torch.clamp(prob, min=(1-self.prob_max)/self.cm.num_ops, max=self.prob_max).to(self.device)
            prob = torch.transpose(prob.t()/torch.sum(prob, dim=1), 0, 1).to(self.device)
        ...
        if self.struc_learning:
            ...
            for idx in range(self.structure_batch):
                ...
                # -- predicted --
                states = torch.Tensor([np.pi for _ in range(self.qdqn.num_qubits)])
                predicted = self.qdqn(states).to(self.device)
                #predicted = predicted.requires_grad_(True)
                #print(predicted)
                # -- target --
                expected_qvalues = TARGET.to(self.device) # TODO original for ccnot

                ## -- loss --
                # loss function在这里是MSE Loss
                sub_loss = self.loss_func(predicted, expected_qvalues).to(self.device)*LOSS_FACTOR
            ...
            with torch.no_grad():
                total_loss = torch.mean(torch.stack(loss_list)).to(self.device)*LOSS_FACTOR
                self.avcost = torch.mean(torch.stack(loss_list)).to(self.device)*LOSS_FACTOR
                if self.debug:
            ...
            predicted = self.qdqn(states).to(self.device)
            #predicted = predicted.requires_grad_(True)
            #predicted = Variable(predicted, requires_grad=True)
            #print(predicted)
            # -- target --
            expected_qvalues = TARGET.to(self.device)
            #print(expected_qvalues)
            ...
            if self.debug:
                ...
                print(f"sub_loss: {sub_loss}]")
            ## -- loss --
            total_loss = self.loss_func(predicted, expected_qvalues).to(self.device)
            #total_loss = total_loss.requires_grad
            #print(total_loss)
            total_loss.backward()
            # for name, param in self.qdqn.named_parameters():
            ...
            if layer_learning_finished:
                ...
                if not self.struc_learning: # learning finish
                    return {
                        ...
                        #'reward': epoch_reward,
                        #'greedy': epoch_greedy,
                        'prob': prob.detach().cpu().clone().numpy() # try.py: prob without all others?
                    }
                else:
    ...
    def learn(self,
        ...
        with tqdm(range(self.total_epochs+1), desc="DQN",
